...
outputI$CIA <- round ((mole.data$Al2O3/(mole.data$Al2O3 + pre_CIA$CaO  + mole.data$Na2O + mole.data$K2O)) * 100, digits =2)
outputI$CIW <- round ((mole.data$Al2O3/(mole.data$Al2O3 + pre_CIA$CaO  + mole.data$Na2O)) * 100, digits =2)
outputI$Mg.number <- round (100 * (mole.data$MgO/(mole.data$MgO + mole.data$FeO)), digits =2)# (Mg#=molar MgO/[molar MgO+molar FeOT
outputI$Fe_star <- round ((as.numeric(data$`FeO adj`) + (0.9*as.numeric(data$`Fe2O3 adj`))) / (as.numeric(data$`FeO adj`) + (0.9*as.numeric(data$`Fe2O3 adj`)) + as.numeric(data$`MgO adj`)), digits =2)
outputI$MALI <-  round (as.numeric(data$`Na2O adj`) + as.numeric(data$`K2O adj`) - as.numeric(data$`CaO adj`), digits = 2)      
outputI$ACNK <- round(((mole.data$Al2O3) / (mole.data$CaO -(1.67*mole.data$P2O5) + mole.data$Na2O + mole.data$K2O)), digits = 2)
outputI$ANK <- round(((mole.data$Al2O3) / (mole.data$Na2O + mole.data$K2O)), digits = 2)
outputI$AI <- round((mole.data$Al2O3) - (mole.data$K2O + mole.data$Na2O), digits = 2)
outputI$tau <- round(log((as.numeric(data$`Al2O3 adj`)-as.numeric(data$`Na2O adj`))/as.numeric(data$`TiO2 adj`)), digits=2) 
outputI$sigma <- round(log(((as.numeric(data$`K2O adj`) + as.numeric(data$`Na2O adj`))**2)/(as.numeric(data$`SiO2 adj`)-43)), digits=2)#(Gottini, 1968);(Rittmann, 1957)
...
outputI$Temp_SiO2 <- round(((-14.16*as.numeric(data$`SiO2 adj`)) + 1723), digits = 2)
outputI$Temp_MgO <- round((887.6*as.numeric(data$`MgO adj`)^(0.0989)), digits = 2)
